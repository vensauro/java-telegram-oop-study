 ...
 * JogadorVilao
 */
public class JogadorVilao extends Usuario implements Jogador {

    private String charactName = "Vilao";

    public JogadorVilao(long id, String nome) {
        super(id, nome);
    }

    public JogadorVilao(long id, String nome, boolean morto) {
        super(id, nome, morto);
    }
    ...
    public void acao(Jogo jogo, Update update, MessageCallback callback) {
        SendMessage message = Utils.sendMessage(this.getId(),
                EmojiParser.parseToUnicode("Quem você quer matar? :gun:"));
        InlineKeyboardMarkup markupInline = new InlineKeyboardMarkup();
        List<List<InlineKeyboardButton>> rowsInline = new ArrayList<>();
        ...
        jogo.getUsuarios().forEach(eachUser -> {
            ...
            List<InlineKeyboardButton> rowInline = new ArrayList<>();
            rowInline.add(new InlineKeyboardButton().setText(nomeAlvo)
                    .setCallbackData("matar " + nomeAlvo));
            rowsInline.add(rowInline);
        });
